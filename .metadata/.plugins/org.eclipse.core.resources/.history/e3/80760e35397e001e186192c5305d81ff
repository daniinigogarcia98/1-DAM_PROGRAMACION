...

{
	...
	public static int[] borarPrimeraOcurrenciav2(int[] arDatos,int dato) {
		int contador=1;
		//Compruebo si el elemento que quiero está en el array
		for(int i=0;i<arDatos.length; i++) {
			if(arDatos[i]==dato) {
	...
	//El elemento a Borrar está en el array
	//Creo un array con un elemento menos que el original
			int[] resultado=new int[arDatos.length-contador];
			int j=0;
			for(int i=0;i<arDatos.length; i++) {
				if(arDatos[i]!=dato)
				{
...
*/
		...
		int[] resb=borarPrimeraOcurrencia(arNumeros,100);
